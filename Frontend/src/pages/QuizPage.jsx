...
const QuizPage = ({ user, setPage }) => {
    ...
    const submitQuiz = async () => {
        ...
        try {
            ...
            setResult({ passed: data.passed, score: data.score, total: data.total, message: data.message });
                ...
                <Card className={`border-l-4 ${result.passed ? 'border-green-500' : 'border-red-500'}`}>
                    <div className="text-center space-y-6 py-8">
                        <div className="flex justify-center">
                            {result.passed ? <CheckCircle className="w-24 h-24 text-green-400 animate-bounce" /> : <XCircle className="w-24 h-24 text-red-400 animate-pulse" />}
                        </div>
                        <div>
                            <h2 className="text-4xl font-bold text-white mb-2">{result.passed ? "Application Approved" : "Application Rejected"}</h2>
                            <p className={`text-xl font-medium ${result.passed ? 'text-green-300' : 'text-red-300'}`}>Score: {result.score} / {result.total}</p>
                        </div>
                        ...
                        <div className="flex justify-center gap-4 pt-4">
                            ...
                            {isAdmin && (
                                <AnimatedButton onClick={() => window.location.reload()} className="bg-purple-600">Admin Retake</AnimatedButton>
                            )}
                            {result.passed && (
                                <AnimatedButton onClick={() => setPage('queue')} className="bg-green-600">Connect to Server</AnimatedButton>
                            )}
                        </div>
                    </div>
                </Card>
        ...
        <div className="max-w-3xl mx-auto pt-10 animate-slide-in-up">
            <Card>
                ...
                <div className="mb-8 min-h-[200px]">
                    ...
                    <div className="space-y-3">
                        {q.options.map((opt, idx) => (
                            <div key={idx} onClick={() => handleAnswer(opt)} className={`p-4 rounded-lg border cursor-pointer transition-all duration-200 flex items-center gap-3 group ${answers[q.question] === opt ? 'bg-cyan-500/20 border-cyan-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.2)] transform scale-[1.02]' : 'bg-gray-800/50 border-gray-700 text-gray-300 hover:bg-gray-700 hover:border-gray-600'}`}>
                ...
                <div className="flex justify-between items-center border-t border-gray-800 pt-6">
                    ...
                    <AnimatedButton onClick={handleNext} disabled={!answers[q.question] || submitting} className="bg-cyan-600 min-w-[140px]">
