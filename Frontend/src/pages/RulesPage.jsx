...
const Rules = () => {
  ...
  const startDrag = (e, direction) => {
    // Only allow dragging if clicking the edge zones (handled by the div placement)
    // We do NOT prevent default here to allow scrolling if the user misses the zone slightly
  ...
  const onDrag = (e) => {
    ...
    // Sensitivity
    const rotationDelta = (deltaX / width) * 180;
  ...
  const endDrag = (e) => {
    ...
    const isClick = Math.abs(dragRotation) < 5;

    if (dragDirection === 'next') {
        if (finalRotation < -15 || isClick) { 
            setCurrentSheet(p => Math.min(p + 1, totalSheets));
        }
    } else {
        if (finalRotation > -165 || isClick) {
  ...
  }, [isDragging, startX, initialRotation, dragRotation, dragDirection]);
    ...
    <div className={`min-h-screen bg-[#050505] flex flex-col items-center justify-center p-4 md:p-8 overflow-hidden font-sans select-none ${isDragging ? 'cursor-grabbing' : ''}`}>
      ...
      >
         ...
         */}
         ...
         {currentSheet > 0 && (
             <div 
                className="absolute left-0 top-0 bottom-0 w-24 md:w-32 z-50 cursor-grab active:cursor-grabbing md:left-[50%] md:-translate-x-[450px]"
         ...
         {currentSheet < totalSheets && (
             <div 
                className="absolute right-0 top-0 bottom-0 w-24 md:w-32 z-50 cursor-grab active:cursor-grabbing md:right-[50%] md:translate-x-[450px]"
...
const PageContainer = ({ children, pageNum, title }) => (
    <div className="h-full flex flex-col p-8 md:p-10 relative bg-[#131316] overflow-hidden">
        {/* Header */}
        <div className="flex justify-between items-center border-b border-white/5 pb-4 mb-6 shrink-0 z-10">
        ...
        {/* Footer Decoration */}
        <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-end shrink-0 opacity-30 z-10">
