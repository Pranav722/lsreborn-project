...
const initializeTables = async () => {
    try {
        // Updated form_settings to include 'type' column
        await db.query(`CREATE TABLE IF NOT EXISTS form_settings (form_name VARCHAR(50) PRIMARY KEY, is_open BOOLEAN DEFAULT true, type VARCHAR(10) DEFAULT 'quiz')`);
...
// New endpoint to get the current Whitelist Form Status (Quiz, Form, or Closed)
...
router.get('/quiz', isAuthenticated, async (req, res) => {
    ...
    if (!settings[0].is_open && !isAdmin) return res.status(403).json({ message: "Whitelist applications are currently disabled.", status: 'disabled' });
    if (settings[0].type !== 'quiz' && !isAdmin) return res.status(403).json({ message: "Whitelist is currently open via the written application.", status: 'form_active' });
